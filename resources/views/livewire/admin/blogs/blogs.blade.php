...
<div x-data x-init="$store.pageName = { name: 'Manage Blogs', slug: 'blogs' }">
        ...
        <div class="grid grid-cols-2 gap-4 px-4 py-4 ">
            ...
            <div>
                <div class="flex gap-4 sm:gap-6 justify-end items-end mt-2">
                    ...
                    <div class="group">
                        <a href="{{ route('admin.blogs.createupdate') }}" type="button"
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    @if ($blogs->count() >= 1)
                        @foreach ($blogs as $blog_item)
                            <tr data-id="{{ $blog_item->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        <img alt=""
                                            src="{{ file_path($blog_item->featured_image) ?? asset('assets/icons/location.png') }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">
                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $blog_item->title }}</p>
                                        {{-- <span class="text-xs text-gray-400">{{ $blog_item->slug }}</span><br> --}}
                                        <span
                                            class="text-xs text-gray-400">{{ $blog_item->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>

                                 <td class="px-3 py-2 whitespace-nowrap">{{ $blog_item->status ? 'Active' : 'Inactive' }}</td>
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">

                                        <a href="{{ route('admin.blogs.createupdate', ['edit' => $blog_item->id]) }}"
