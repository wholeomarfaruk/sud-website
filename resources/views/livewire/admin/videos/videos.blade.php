...
<div x-data x-init="$store.pageName = { name: 'Featured Videos', slug: 'featured-videos' }">
        ...
        <div class="grid grid-cols-2 gap-4 px-4 py-4 ">
            ...
            <div>
                <div class="flex gap-4 sm:gap-6 justify-end items-end mt-2">
                    ...
                    <div class="group">
                        <button wire:click="addVideoModalOpen=true" type="button"
                ...
                <tbody class="divide-y divide-gray-200 [&>*:nth-child(-n+4)]:border-b-2 [&>*:nth-child(-n+4)]:border-green-500" id="sortable-table" wire:ignore>
                    ...
                    @if ($videos && $videos->count() >= 1)
                        @foreach ($videos as $video_item)
                            <tr data-id="{{ $video_item->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        ...
                                        <img alt="" src="{{ file_path($video_item->thumbnail) }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">
                                            </a>

                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $video_item->title }}</p>
                                        <span class="text-xs text-gray-400">{{ gmdate('i:s', $video_item->duration) }}</span><br>
                                        <span
                                            class="text-xs text-gray-400">{{ $video_item->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>
                                            ...
                                            @click="
                                                Swal.fire({
                                                    ...
                                                    if (result.isConfirmed) {
                                                        $wire.delete({{ $video_item->id }})
                                                    }
                                                })
                                            "
                                            href="{{ route('admin.sliders.create', ['edit' => $video_item->id]) }}"
                            ...
                            @if ($video_thumbnail)
                                ...
                                @else
                                    <div class="relative inline-block m-2">
                                        <img src="{{ file_path($video_thumbnail) }}" class="w-full max-h-10  rounded border">
                                        <button type="button"
