...
<div x-data x-init="$store.pageName = { name: 'Manage Properties', slug: 'properties' }">
            ...
            <div>
                <div class="flex gap-4 sm:gap-6 justify-end items-end mt-2">
                    <div class="flex flex-col">
                        ...
                        <select wire:model.live="location" name="status" class="shadow-sm border border-gray-300 rounded px-3 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select wire:model.live="status" name="status" class="shadow-sm border border-gray-300 rounded px-3 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                            <option value="">All</option>
                    ...
                    <div class="group">
                        <a href="{{ route('admin.properties.create') }}" type="button"
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    @if ($properties->count() >= 1)
                        @foreach ($properties as $property_item)
                            <tr data-id="{{ $property_item->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        <img alt=""
                                            src="{{ file_path($property_item->featured_image_id) ?? asset('assets/icons/location.png') }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">
                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $property_item->title }}</p>
                                        <span class="text-xs text-gray-400">{{ $property_item->slug }}</span><br>
                                        <span
                                            class="text-xs text-gray-400">{{ $property_item->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>
                                 <td class="px-3 py-2 whitespace-nowrap">{{ $property_item->getLocation->name ?? 'No Location' }}</td>
                                 <td class="px-3 py-2 whitespace-nowrap">{{ $property_item->status ? 'Active' : 'Inactive' }}</td>
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">

                                        <a href="{{ route('admin.properties.create', ['edit' => $property_item->id]) }}"
                                            ...
                                            @click="
                                                Swal.fire({
                                                    ...
                                                    if (result.isConfirmed) {
                                                        $wire.delete({{ $property_item->id }})
                                                    }
                                                })
                                            "
                                            href="{{ route('admin.sliders.create', ['edit' => $property_item->id]) }}"
