...
<div x-data x-init="$store.pageName = { name: 'Manage Offers', slug: 'offers' }">
        ...
        <div class="grid grid-cols-2 gap-4 px-4 py-4 ">
            ...
            <div>
                <div class="flex gap-4 sm:gap-6 justify-end items-end mt-2">
                    ...
                    <div class="group">
                        <a href="{{ route('admin.offers.createupdate') }}" type="button"
                            ...
                            <span class="text-sm font-medium"> Add New Offer</span>
                        </a>
                    </div>
                </div>
            </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    @if ($offers->count() >= 1)
                        @foreach ($offers as $offer_item)
                            <tr data-id="{{ $offer_item->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        <img alt=""
                                            src="{{ file_path($offer_item->featured_image) ?? asset('assets/icons/location.png') }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">
                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $offer_item->title }}</p>
                                        {{-- <span class="text-xs text-gray-400">{{ $offer_item->slug }}</span><br> --}}
                                        <span
                                            class="text-xs text-gray-400">{{ $offer_item->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>

                                 <td class="px-3 py-2 whitespace-nowrap">{{ $offer_item->status ? 'Active' : 'Inactive' }}</td>
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">

                                        <a href="{{ route('admin.offers.createupdate', ['edit' => $offer_item->id]) }}"
