                ...
                <tbody class="divide-y divide-gray-200 " id="sortable-table" wire:ignore>
                    ...
                    @if ($partners && $partners->count() >= 1)
                        @foreach ($partners as $partner_item)
                            <tr data-id="{{ $partner_item->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        <a href="{{ file_path($partner_item->image_id) }}"  data-fancybox>
                                        <img alt="" src="{{ file_path($partner_item->image_id) }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">
                                            </a>

                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $partner_item->name }}</p>
                                        <span class="text-xs text-gray-400">{{ Str::limit($partner_item->link, 20) }}<br></span>
                                        <span
                                            class="text-xs text-gray-400">{{ $partner_item->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>

                                <td class="px-3 py-2 whitespace-nowrap">
                                    {{ $partner_item->status ? 'Active' : 'Inactive' }}
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">

<button wire:click="edit({{ $partner_item->id }})"
                                        ...
                                        <button x-data
                                            ...
                                            "
                                            href="{{ route('admin.sliders.create', ['edit' => $partner_item->id]) }}"
 ...
 <div x-cloak x-data="{ modalOpen: @entangle('modalOpen') }" x-show="modalOpen" x-transition
        ...
        <div class="w-full md:w-md  rounded-lg bg-white p-6 shadow-lg overflow-y-scroll">
            <div class="flex items-start justify-between">
                <h2 id="modalTitle" class="text-xl font-bold text-gray-900 sm:text-2xl">Add New Partner</h2>

                <button wire:click="modalOpen=false" type="button"
            ...
            <div class="mt-4 overflow-auto">
                ...
                <form action="#" class="space-y-4" wire:submit.prevent="save">
                    <div class="grid grid-cols-1 gap-1">
                        <label class="block text-sm font-medium text-gray-900" for="name">Partner Name</label>
                        <input wire:model="name"
                            class="mt-1 w-full rounded-lg border border-gray-300 focus:border-indigo-500 focus:outline-none p-2"
                            id="name" type="text" placeholder="Enter Partner name" />
                        @error('name')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="grid grid-cols-1 gap-1">
                        <label class="block text-sm font-medium text-gray-900" for="link">Partner's link</label>
                        <input wire:model="link"
                            class="mt-1 w-full rounded-lg border border-gray-300 focus:border-indigo-500 focus:outline-none p-2"
                            id="link" type="text" placeholder="Enter Url" />
                        @error('link')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-1 mb-2">
                        <label class="block text-sm font-medium text-gray-900" for="image_id">Image
                        ...
                        grid  place-content-center {{ $image_id ? '' : ' cursor-pointer' }}">
                            @if ($image_id)
                                @if (is_array($image_id))
                                    @foreach ($image_id as $image_id_item)
                                        <div class="relative inline-block m-2 h-25">
                                            <img src="{{ file_path($image_id_item) }}"
                                                class="h-full rounded border replace-video-preview">
                                            <button type="button"
                                                wire:click="removeMedia('image_id', '{{ $image_id_item }}')"
                                ...
                                @else
                                    <div class="relative inline-block m-2">
                                        <img src="{{ file_path($image_id) }}" class="w-full max-h-10  rounded border">
                                        <button type="button"
