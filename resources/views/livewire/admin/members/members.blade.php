                ...
                <tbody class="divide-y divide-gray-200 " id="sortable-table" wire:ignore>
                    ...
                    @if ($members && $members->count() >= 1)
                        @foreach ($members as $member)
                            <tr data-id="{{ $member->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        ...
                                        <img alt="" src="{{ file_path($member->image) }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">
                                            </a>

                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $member->name }}</p>
                                        <span class="text-xs text-gray-400">{{ $member->designation }}<br></span>
                                        <span
                                            class="text-xs text-gray-400">{{ $member->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>
                                        ...
                                        <button x-data
                                            @click="
                                                Swal.fire({
                                                    ...
                                                    confirmButtonText: 'Yes, delete board member!'
                                                }).then((result) => {
                                            ...
                                            "
                                            href="{{ route('admin.sliders.create', ['edit' => $member->id]) }}"
        ...
        <div class="w-full md:w-md  rounded-lg bg-white p-6 shadow-lg overflow-y-scroll">
            ...
            <div class="mt-4 overflow-auto">
                ...
                <form action="#" class="space-y-4" wire:submit.prevent="save">
                    ...
                    <div class="grid grid-cols-1 gap-1">
                        <label class="block text-sm font-medium text-gray-900" for="designation">Designation</label>
                            ...
                            @if ($image)
                                ...
                                @else
                                    <div class="relative inline-block m-2">
                                        <img src="{{ file_path($image) }}" class="w-full max-h-10  rounded border">
                                        <button type="button"
                                            wire:click="removeMedia('image', '{{ $image }}')"
                                            class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 text-xs flex items-center justify-center">
                                            ✕
                                        </button>
                                    </div>
