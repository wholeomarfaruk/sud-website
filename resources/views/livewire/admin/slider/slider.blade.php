...
<div x-data x-init="$store.pageName = { name: 'Slider', slug: 'slider' }">
                ...
                <div class="flex gap-4 sm:gap-6 justify-end items-end mt-2">
                    ...
                    <div class="group">
                        <a href="{{ route('admin.sliders.create') }}" type="button"
                            ...
                            <span class="text-sm font-medium"> Add New Slide</span>
                        </a>
                    </div>
                </div>
            </div>
                ...
                <tbody class="divide-y divide-gray-200" id="sortable-table" wire:ignore>
                    ...
                    @if ($sliders->count() >= 1)
                        @foreach ($sliders as $slider_item)
                            <tr data-id="{{ $slider_item->id }}" class="*:text-gray-900 *:first:font-medium">
                                <td class="px-3 py-2 whitespace-nowrap flex justify-start gap-2 items-center">
                                    ...
                                    <div class=" sm:shrink-0">
                                        <img alt="" src="{{ file_path($slider_item->image_id) }}"
                                            class="size-12 rounded-lg object-cover sm:size-[52px]">

                                    </div>
                                    <div>
                                        <p class="font-bold mb-0 text-sm/2">{{ $slider_item->title }}</p>
                                        {{-- <span class="text-xs text-gray-400">#{{ $slider_item->sort_order }}</span><br> --}}
                                        <span
                                            class="text-xs text-gray-400">{{ $slider_item->updated_at->format('d-m-Y h:i A') }}</span>
                                    </div>
                                </td>

                                <td class="px-3 py-2 whitespace-nowrap">
                                    <p class="text-sm text-gray-900">
                                        {{ Str::limit($slider_item->link, 20) ?? ($slider_item->link ?? 'No Link') }}</p>

                                </td>
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <p class="text-sm text-gray-900 px-2 py-1 rounded-2xl">
                                        {{ $slider_item->status == 1 ? 'Active' : 'Inactive' }}
                                    </p>
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button x-data
                                            ...
                                            "
                                            href="{{ route('admin.sliders.create', ['edit' => $slider_item->id]) }}"
